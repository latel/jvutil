...
exports.default = exports.toCurrency = exports.toText = exports.fen2yuan = exports.yuan2fen = void 0;
...
var yuan2fen = function yuan2fen(val) {
  // 强制转换成字符串
  val = String(val);
...
var fen2yuan = function fen2yuan(val) {
  // 强制转换成字符串
  val = String(val);
  return REGEX_A_NUMBER.test(val) ? (parseFloat(val) / 100).toFixed(2) : "0.00";
};
...
var toText = function toText(val, decimal, unit, level) {
  val = Number(val) || 0; // 默认转换万以上的数字

  level = level || 10000; // 默认保留两位小数

  decimal = decimal === undefined ? 2 : decimal; // 默认单位为空
...
var toCurrency = function toCurrency(val, decimal) {
  val = String(val) || "";
